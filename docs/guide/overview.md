...
- **Billing**: Laravel Cashier (Stripe) (see [Plans Module](plans-module.md))
...
- A **service set** (see [Service Layer](service-layer.md))
- Shared **Action classes** injected into the services (see [Actions](actions.md))
- **Audit logging** wiring via the `Auditable` contract (see [Audit Logging](audit-logging.md))
- A **Policy** that delegates to `PolicyAuthorisationService` (see [Authorisation](authorisation.md))
- Grouped, RESTful-plus-bulk **routes** (see [Routes](routes.md))
- Vue pages (`Index`, `Create`, `Edit`, `Show`) built from small, reusable **components** (see [Frontend: Vue Patterns](frontend-vue-patterns.md))
- A model interface in `types/index.ts` (see [Frontend: TypeScript Types](frontend-types.md))
...
See [Example Models](example-models.md) for the full, real `Company` and `User` models showing every one of these conventions together.
...
| Route grouping | `Route::prefix('{resource}')->name('{resource}.')->group(...)` (see [Routes](routes.md)) |
