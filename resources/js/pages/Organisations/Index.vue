...
function applyFilters(): void {
    router.get(organisationsIndex.url(), filters.value, {
        preserveState: true,
        replace: true,
    });
}
...
function destroy(): void {
    ...
    router.delete(organisationsDestroy.url(selectedOrganisationId.value), {
        preserveScroll: true,
        onFinish: () => {
...
<template>
    <div class="py-6">
        <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            ...
            >
                ...
                <template #actions="{ row }">
                    <Link :href="organisationsShow.url(row.id)">View</Link>
                    <Link :href="organisationsEdit.url(row.id)">Edit</Link>
                    <button
                        type="button"
        ...
        <ConfirmDialog
            ...
            :description="`${pendingBulkIds.length} organisation(s) will be moved to trash.`"
            confirm-label="Delete"
