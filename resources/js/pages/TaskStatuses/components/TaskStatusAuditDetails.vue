    ...
    <div class="rounded-lg border p-4">
        ...
        <dl class="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <div>
                ...
                <dd class="text-sm">
                    {{ taskStatus.creator?.name ?? 'System' }}
                </dd>
            </div>
            <div>
                <dt class="text-xs text-gray-400">Created at</dt>
                <dd class="text-sm">
                    {{ formatDateTime(taskStatus.created_at) }}
                </dd>
            </div>
            ...
            <div>
                ...
                <dd class="text-sm">
                    {{ taskStatus.updater?.name ?? 'System' }}
                </dd>
            </div>
            <div>
                <dt class="text-xs text-gray-400">Last updated at</dt>
                <dd class="text-sm">
                    {{ formatDateTime(taskStatus.updated_at) }}
                </dd>
            </div>
            ...
            <template v-if="taskStatus.deleted_at">
                <div>
                    ...
                    <dd class="text-sm">
                        {{ taskStatus.deleter?.name ?? 'System' }}
                    </dd>
                </div>
                <div>
            ...
            <template v-if="taskStatus.restored_at">
                <div>
                    ...
                    <dd class="text-sm">
                        {{ taskStatus.restorer?.name ?? 'System' }}
                    </dd>
                </div>
                <div>
