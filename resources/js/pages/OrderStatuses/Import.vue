...
import { preview as importPreview, commit as importCommit } from '@/routes/order-statuses/import';

defineOptions({
    layout: { breadcrumbs: [{ title: 'Order Statuses', href: orderStatusesIndex().url }, { title: 'Import' }] },
});
...
async function onConfirmCommit(): Promise<void> {
    ...
    if (result) {
        router.visit(orderStatusesIndex().url, {
...
<template>
    <div class="space-y-6 p-6">
        <h1 class="text-lg font-semibold text-gray-200">Import order statuses</h1>
