    ...
    <div class="rounded-lg border p-4">
        ...
        <dl class="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <div>
                ...
                <dd class="text-sm">
                    {{ orderStatus.creator?.name ?? 'System' }}
                </dd>
            </div>
            <div>
                <dt class="text-xs text-gray-400">Created at</dt>
                <dd class="text-sm">
                    {{ formatDateTime(orderStatus.created_at) }}
                </dd>
            </div>
            ...
            <div>
                ...
                <dd class="text-sm">
                    {{ orderStatus.updater?.name ?? 'System' }}
                </dd>
            </div>
            <div>
                <dt class="text-xs text-gray-400">Last updated at</dt>
                <dd class="text-sm">
                    {{ formatDateTime(orderStatus.updated_at) }}
                </dd>
            </div>
            ...
            <template v-if="orderStatus.deleted_at">
                <div>
                    ...
                    <dd class="text-sm">
                        {{ orderStatus.deleter?.name ?? 'System' }}
                    </dd>
                </div>
                <div>
            ...
            <template v-if="orderStatus.restored_at">
                <div>
                    ...
                    <dd class="text-sm">
                        {{ orderStatus.restorer?.name ?? 'System' }}
                    </dd>
                </div>
                <div>
