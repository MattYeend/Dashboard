...
import { index as itemsIndex } from '@/routes/invoices/items';
import { preview as importPreview, commit as importCommit } from '@/routes/invoices/items/import';
...
defineOptions({
    layout: { breadcrumbs: [{ title: 'Invoice items', href: itemsIndex(props.invoiceId).url }, { title: 'Import' }] },
});
...
const { preview, processing, error, runPreview, runCommit, reset } = useImportWorkflow(
    importPreview(props.invoiceId).url,
    importCommit(props.invoiceId).url,
);
...
async function onConfirmCommit(): Promise<void> {
    ...
    if (result) {
        router.visit(itemsIndex(props.invoiceId).url, {
