...
import { preview as importPreview, commit as importCommit } from '@/routes/contacts/import';

defineOptions({
    layout: { breadcrumbs: [{ title: 'Contacts', href: contactsIndex().url }, { title: 'Import' }] },
});
...
async function onConfirmCommit(): Promise<void> {
    ...
    if (result) {
        router.visit(contactsIndex().url, {
...
<template>
    <div class="space-y-6 p-6">
        <h1 class="text-lg font-semibold text-gray-200">Import contacts</h1>
