    ...
    <div class="rounded-lg border p-4">
        ...
        <dl class="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <div>
                ...
                <dd class="text-sm">{{ company.creator?.name ?? '-' }}</dd>
            </div>
            <div>
                <dt class="text-xs text-gray-400">Created at</dt>
                <dd class="text-sm">{{ formatDateTime(company.created_at) }}</dd>
            </div>

            <div>
                <dt class="text-xs text-gray-400">Last updated by</dt>
                <dd class="text-sm">{{ company.updater?.name ?? '-' }}</dd>
            </div>
            <div>
                <dt class="text-xs text-gray-400">Last updated at</dt>
                <dd class="text-sm">{{ formatDateTime(company.updated_at) }}</dd>
            </div>

            <template v-if="company.deleted_at">
                <div>
                    <dt class="text-xs text-gray-400">Deleted by</dt>
                    <dd class="text-sm">{{ company.deleter?.name ?? '-' }}</dd>
                </div>
                <div>
            ...
            <template v-if="company.restored_at">
                <div>
                    <dt class="text-xs text-gray-400">Restored by</dt>
                    <dd class="text-sm">{{ company.restorer?.name ?? '-' }}</dd>
                </div>
                <div>
                    <dt class="text-xs text-gray-400">Restored at</dt>
                    <dd class="text-sm">
                        {{ formatDateTime(company.restored_at) }}
                    </dd>
                </div>
            </template>
        </dl>
