    ...
    <div class="rounded-lg border p-4">
        ...
        <dl class="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <div>
                ...
                <dd class="text-sm">{{ user.creator?.name ?? 'System' }}</dd>
            </div>
            <div>
                <dt class="text-xs text-gray-400">Created at</dt>
                <dd class="text-sm">{{ formatDateTime(user.created_at) }}</dd>
            </div>

            <div>
                <dt class="text-xs text-gray-400">Last updated by</dt>
                <dd class="text-sm">{{ user.updater?.name ?? 'System' }}</dd>
            </div>
            <div>
                <dt class="text-xs text-gray-400">Last updated at</dt>
                <dd class="text-sm">{{ formatDateTime(user.updated_at) }}</dd>
            </div>
            ...
            <template v-if="user.restored_at">
                ...
                <div>
                    ...
                    <dd class="text-sm">
                        {{ formatDateTime(user.restored_at) }}
                    </dd>
                </div>
            </template>
        </dl>
