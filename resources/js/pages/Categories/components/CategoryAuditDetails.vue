    ...
    <div class="rounded-lg border p-4">
        ...
        <dl class="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <div>
                ...
                <dd class="text-sm">{{ category.creator?.name ?? '-' }}</dd>
            </div>
            <div>
                <dt class="text-xs text-gray-400">Created at</dt>
                <dd class="text-sm">{{ formatDateTime(category.created_at) }}</dd>
            </div>

            <div>
                <dt class="text-xs text-gray-400">Last updated by</dt>
                <dd class="text-sm">{{ category.updater?.name ?? '-' }}</dd>
            </div>
            <div>
                <dt class="text-xs text-gray-400">Last updated at</dt>
                <dd class="text-sm">{{ formatDateTime(category.updated_at) }}</dd>
            </div>

            <template v-if="category.deleted_at">
                <div>
                    <dt class="text-xs text-gray-400">Deleted by</dt>
                    <dd class="text-sm">{{ category.deleter?.name ?? '-' }}</dd>
                </div>
                <div>
            ...
            <template v-if="category.restored_at">
                <div>
                    ...
                    <dd class="text-sm">{{ category.restorer?.name ?? '-' }}</dd>
                </div>
                <div>
                    <dt class="text-xs text-gray-400">Restored at</dt>
                    <dd class="text-sm">
                        {{ formatDateTime(category.restored_at) }}
                    </dd>
                </div>
            </template>
        </dl>
