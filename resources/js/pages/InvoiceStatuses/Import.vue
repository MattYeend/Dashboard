...
import { preview as importPreview, commit as importCommit } from '@/routes/invoice-statuses/import';

defineOptions({
    layout: { breadcrumbs: [{ title: 'Invoice Statuses', href: invoiceStatusesIndex().url }, { title: 'Import' }] },
});
...
async function onConfirmCommit(): Promise<void> {
    ...
    if (result) {
        router.visit(invoiceStatusesIndex().url, {
...
<template>
    <div class="space-y-6 p-6">
        <h1 class="text-lg font-semibold text-gray-200">Import invoice statuses</h1>
