...
import { preview as importPreview, commit as importCommit } from '@/routes/pipeline-statuses/import';

defineOptions({
    layout: { breadcrumbs: [{ title: 'Pipeline Statuses', href: pipelineStatusesIndex().url }, { title: 'Import' }] },
});
...
async function onConfirmCommit(): Promise<void> {
    ...
    if (result) {
        router.visit(pipelineStatusesIndex().url, {
...
<template>
    <div class="space-y-6 p-6">
        <h1 class="text-lg font-semibold text-gray-200">Import pipeline statuses</h1>
