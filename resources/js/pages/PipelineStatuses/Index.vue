...
const selectedPipelineStatusId = ref<number | null>(null);
...
const filterFields = [
    {
        key: 'search',
        type: 'text' as const,
        placeholder: 'Search pipeline statuses…',
    },
    {
        key: 'trashed',
...
function applyFilters(): void {
    router.get(pipelineStatusesIndex.url(), filters.value, {
        preserveState: true,
        replace: true,
    });
}
...
function requestDestroy(id: number): void {
    selectedPipelineStatusId.value = id;
    deleteDialogOpen.value = true;
}

function destroy(): void {
    if (selectedPipelineStatusId.value === null) {
        return;
    }

    deleteProcessing.value = true;

    router.delete(pipelineStatusesDestroy.url(selectedPipelineStatusId.value), {
        preserveScroll: true,
        onFinish: () => {
            deleteProcessing.value = false;
            deleteDialogOpen.value = false;
            selectedPipelineStatusId.value = null;
        },
    });
}
...
function bulkDelete(): void {
    ...
    router.post(
        pipelineStatusesBulk.delete.url(),
    ...
    <div class="py-6">
        <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            ...
            <ResourceTable
                ...
                :rows="pipelineStatuses.data"
                :columns="columns"
                row-key="id"
                selectable
                empty-message="No pipeline statuses found."
            >
                ...
                <template #actions="{ row }">
                    <Link :href="pipelineStatusesShow.url(row.id)">View</Link>
                    <Link :href="pipelineStatusesEdit.url(row.id)">Edit</Link>
                    <button
                        type="button"
        ...
        <ConfirmDialog
            ...
            :description="`${pendingBulkIds.length} pipeline status(es) will be moved to trash.`"
            confirm-label="Delete"
