...
import { preview as importPreview, commit as importCommit } from '@/routes/pipelines/import';

defineOptions({
    layout: { breadcrumbs: [{ title: 'Pipelines', href: pipelinesIndex().url }, { title: 'Import' }] },
});
...
async function onConfirmCommit(): Promise<void> {
    ...
    if (result) {
        router.visit(pipelinesIndex().url, {
...
<template>
    <div class="space-y-6 p-6">
        <h1 class="text-lg font-semibold text-gray-200">Import pipelines</h1>
