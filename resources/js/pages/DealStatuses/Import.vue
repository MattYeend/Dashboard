...
import { preview as importPreview, commit as importCommit } from '@/routes/deal-statuses/import';

defineOptions({
    layout: { breadcrumbs: [{ title: 'Deal Statuses', href: dealStatusesIndex().url }, { title: 'Import' }] },
});
...
async function onConfirmCommit(): Promise<void> {
    ...
    if (result) {
        router.visit(dealStatusesIndex().url, {
...
<template>
    <div class="space-y-6 p-6">
        <h1 class="text-lg font-semibold text-gray-200">Import deal statuses</h1>
