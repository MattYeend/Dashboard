...
const selectedDealStatusId = ref<number | null>(null);
...
const filterFields = [
    {
        key: 'search',
        type: 'text' as const,
        placeholder: 'Search deal statuses…',
    },
    {
        key: 'trashed',
...
function applyFilters(): void {
    router.get(dealStatusesIndex.url(), filters.value, {
        preserveState: true,
        replace: true,
    });
}
...
function requestDestroy(id: number): void {
    selectedDealStatusId.value = id;
    deleteDialogOpen.value = true;
}

function destroy(): void {
    if (selectedDealStatusId.value === null) {
        return;
    }

    deleteProcessing.value = true;

    router.delete(dealStatusesDestroy.url(selectedDealStatusId.value), {
        preserveScroll: true,
        onFinish: () => {
            deleteProcessing.value = false;
            deleteDialogOpen.value = false;
            selectedDealStatusId.value = null;
        },
    });
}
    ...
    <div class="py-6">
        <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            ...
            <ResourceTable
                ...
                :rows="dealStatuses.data"
                :columns="columns"
                row-key="id"
                selectable
                empty-message="No deal statuses found."
            >
                ...
                <template #actions="{ row }">
                    <Link :href="dealStatusesShow.url(row.id)">View</Link>
                    <Link :href="dealStatusesEdit.url(row.id)">Edit</Link>
                    <button
                        type="button"
        ...
        <ConfirmDialog
            ...
            :description="`${pendingBulkIds.length} deal status(es) will be moved to trash.`"
            confirm-label="Delete"
