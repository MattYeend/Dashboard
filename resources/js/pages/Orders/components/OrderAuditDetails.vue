    ...
    <div class="rounded-lg border p-4">
        ...
        <dl class="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <div>
                ...
                <dd class="text-sm">{{ order.creator?.name ?? '-' }}</dd>
            </div>
            <div>
                <dt class="text-xs text-gray-400">Created at</dt>
                <dd class="text-sm">{{ formatDateTime(order.created_at) }}</dd>
            </div>

            <div>
                <dt class="text-xs text-gray-400">Last updated by</dt>
                <dd class="text-sm">{{ order.updater?.name ?? '-' }}</dd>
            </div>
            <div>
                <dt class="text-xs text-gray-400">Last updated at</dt>
                <dd class="text-sm">{{ formatDateTime(order.updated_at) }}</dd>
            </div>

            <template v-if="order.deleted_at">
                <div>
                    <dt class="text-xs text-gray-400">Deleted by</dt>
                    <dd class="text-sm">{{ order.deleter?.name ?? '-' }}</dd>
                </div>
                <div>
            ...
            <template v-if="order.restored_at">
                <div>
                    ...
                    <dd class="text-sm">{{ order.restorer?.name ?? '-' }}</dd>
                </div>
                <div>
                    <dt class="text-xs text-gray-400">Restored at</dt>
                    <dd class="text-sm">
                        {{ formatDateTime(order.restored_at) }}
                    </dd>
                </div>
            </template>
        </dl>
