    ...
    <div class="rounded-lg border p-4">
        ...
        <dl class="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <div>
                ...
                <dd class="text-sm">
                    {{ ticketPriority.creator?.name ?? 'System' }}
                </dd>
            </div>
            <div>
                <dt class="text-xs text-gray-400">Created at</dt>
                <dd class="text-sm">
                    {{ formatDateTime(ticketPriority.created_at) }}
                </dd>
            </div>
            ...
            <div>
                ...
                <dd class="text-sm">
                    {{ ticketPriority.updater?.name ?? 'System' }}
                </dd>
            </div>
            <div>
                <dt class="text-xs text-gray-400">Last updated at</dt>
                <dd class="text-sm">
                    {{ formatDateTime(ticketPriority.updated_at) }}
                </dd>
            </div>

            <template v-if="ticketPriority.deleted_at">
                <div>
                    <dt class="text-xs text-gray-400">Deleted by</dt>
                    <dd class="text-sm">
                        {{ ticketPriority.deleter?.name ?? 'System' }}
                    </dd>
                </div>
                <div>
                    <dt class="text-xs text-gray-400">Deleted at</dt>
                    <dd class="text-sm">
                        {{ formatDateTime(ticketPriority.deleted_at) }}
                    </dd>
                </div>
            </template>

            <template v-if="ticketPriority.restored_at">
                <div>
                    <dt class="text-xs text-gray-400">Restored by</dt>
                    <dd class="text-sm">
                        {{ ticketPriority.restorer?.name ?? 'System' }}
                    </dd>
                </div>
                <div>
                    <dt class="text-xs text-gray-400">Restored at</dt>
                    <dd class="text-sm">
                        {{ formatDateTime(ticketPriority.restored_at) }}
                    </dd>
                </div>
            </template>
        </dl>
