...
import { preview as importPreview, commit as importCommit } from '@/routes/ticket-priorities/import';

defineOptions({
    layout: { breadcrumbs: [{ title: 'Ticket Priorities', href: ticketPrioritiesIndex().url }, { title: 'Import' }] },
});
...
async function onConfirmCommit(): Promise<void> {
    ...
    if (result) {
        router.visit(ticketPrioritiesIndex().url, {
...
<template>
    <div class="space-y-6 p-6">
        <h1 class="text-lg font-semibold text-gray-200">Import ticket priorities</h1>
