...
class PipelineStageController extends Controller
{
     ...
     * without persisting anything, scoped to one pipeline.
     *
     * Authorisation is handled by ImportPipelineStageRequest::authorize().
     */
    public function importPreview(ImportPipelineStageRequest $request, Pipeline $pipeline): JsonResponse
    {
        $importer = $this->management->importer();
        $token = $importer->storeUpload($request->file('file'), $request->user()->id, ['pipeline' => $pipeline]);
     ...
     */
    public function importCommit(Request $request, Pipeline $pipeline): JsonResponse
    {
        $this->authorize('import', PipelineStage::class);
