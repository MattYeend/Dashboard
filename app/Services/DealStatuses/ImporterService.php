...
/**
 * Handles CSV import of deal statuses, including the newer
...
class ImporterService
{
    ...
    {
        return Log::ACTION_IMPORT_DEAL_STATUS;
    }

    /**
     * Persist a single validated row as a new DealStatus.
     *
        ...
        DB::transaction(function () use ($handle, $header, $actor, $actorId, &$imported, &$skipped, &$rowNumber) {
            while (($row = fgetcsv($handle)) !== false) {
                ...
                $this->auditLogService->record(
                    Log::ACTION_IMPORT_DEAL_STATUS,
                    $actor,
                    $dealStatus,
                    ['after' => $this->auditLogService->snapshot($dealStatus)],
                );
