...
/**
 * Handles CSV import of ticket statuses, including the newer
...
class ImporterService
{
    ...
    {
        return 'imports/ticket-statuses';
    }
    ...
    {
        return Log::ACTION_IMPORT_TICKET_STATUS;
    }

    /**
     * Persist a single validated row as a new TicketStatus.
     *
        ...
        DB::transaction(function () use ($handle, $header, $actor, $actorId, &$imported, &$skipped, &$rowNumber) {
            while (($row = fgetcsv($handle)) !== false) {
                ...
                $this->auditLogService->record(
                    Log::ACTION_IMPORT_TICKET_STATUS,
                    $actor,
                    $ticketStatus,
                    ['after' => $this->auditLogService->snapshot($ticketStatus)],
                );
