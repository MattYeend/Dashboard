...
/**
 * Handles CSV import of pipeline statuses, including the newer
...
class ImporterService
{
    ...
    {
        return 'imports/pipeline-statuses';
    }
    ...
    {
        return Log::ACTION_IMPORT_PIPELINE_STATUS;
    }

    /**
     * Persist a single validated row as a new PipelineStatus.
     *
    ...
    public function import(
        ...
        DB::transaction(function () use ($handle, $header, $actor, $actorId, &$imported, &$skipped, &$rowNumber) {
            while (($row = fgetcsv($handle)) !== false) {
                ...
                $pipelineStatus = PipelineStatus::create([
                    'title' => $data['title'],
                    'description' => $data['description'] ?? null,
                    'background_colour' => strtoupper(trim($data['background_colour'])),
                    'text_colour' => strtoupper(trim($data['text_colour'])),
                ...
                $this->auditLogService->record(
                    Log::ACTION_IMPORT_PIPELINE_STATUS,
                    $actor,
                    $pipelineStatus,
                    ['after' => $this->auditLogService->snapshot($pipelineStatus)],
                );
